...
var featuredcontentslider = {
	...
	buildpaginate : function(b) {
		...
		if (typeof i == "string" && i != "markup" && i != "scroll"
				|| typeof i == "object") {
			for ( var c = 1; c <= b.contentdivs.length; c++) {
				j += '<i class="iToc iToc'
						...
						+ c
						+ '" class="toc">'
						+ (typeof i == "string" ? i.replace(/#increment/, c)
								: i[c - 1]) + '</a></i> '
			}
			j = (d[0] != '' ? '<i class="iToc iToc' + c
					+ '"><a href="#prev" class="prev">' + d[0] + '</a></i> '
					: '')
					+ j
					+ (d[1] != '' ? '<i class="iToc iToc' + c
							+ '"><a href="#next" class="next">' + d[1]
							+ '</a></i>' : '');
		...
		for ( var c = 0; c < h.length; c++) {
			if (this.css(h[c], "toc", "check")) {
				...
				if (i == "scroll") {
					...
					l.push(l[l.length - 1] + g.offsetWidth
							+ this.style(g, "marginLeft", true)
							+ this.style(g, "marginRight", true)
							+ this.style(g, "borderLeftWidth", true)
							+ this.style(g, "borderRightWidth", true))
				}
				...
				b.toclinks.push(h[c])
			} else if (this.css(h[c], "prev", "check")
					|| this.css(h[c], "next", "check")) {
		...
		if (b.autorotate[0]) {
			...
			e["onmouseover"] = f["onmouseover"] = function() {
				featuredcontentslider
						.cleartimer(b, window["fcsautorun" + b.id]);
				return true
			};
	...
	turnpage : function(a, b, e) {
		var f = a.currentpage, j = a.contentdivs, i = j.length, d = (/prev/i
				.test(b)) ? f - 1 : (/next/i.test(b)) ? f + 1 : b | 0;
		d = (d < 1) ? i : (d > i) ? 1 : d;
		...
		a.prevpage = d;
		if (a.toc == "scroll") {
			var c = this, h = 0.25, m = 20, k = c.masker.clientWidth, n = a.contentdivs.length, o = a.toclinks[d - 1], l = this.thumbWidth[this.thumbWidth.length - 1], d = k
					/ 2 - o.offsetWidth / 2 - this.thumbWidth[d - 1], g = c.maskee.currentStyle ? c.maskee.currentStyle["marginLeft"]
					: window.getComputedStyle(c.maskee, "").getPropertyValue(
							"margin-left");
	...
	setopacity : function(a, b) {
		...
		e.style.cssText += ";filter:alpha(opacity=" + b * 100
				+ ");-moz-opacity:" + b + ";opacity:" + b;
		a.curopacity = b
	},
	fadeup : function(a) {
